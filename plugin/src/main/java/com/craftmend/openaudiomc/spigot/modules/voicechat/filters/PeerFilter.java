...
@NoArgsConstructor
public class PeerFilter extends Filter<ClientConnection, PlayerEntity> {
     ...
     */
    @Override
    public Stream<ClientConnection> wrap(Stream<ClientConnection> existingStream, PlayerEntity context) {
        Stream<ClientConnection> s = existingStream.filter(possiblePeer -> {
            ...
            if (!otherPlayer.getWorld().equals(context.getWorld()))
                return false;
