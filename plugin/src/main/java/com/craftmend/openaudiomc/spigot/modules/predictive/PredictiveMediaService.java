...
public class PredictiveMediaService extends Service {
        ...
        return new INetworkingEvents() {
            ...
            public void onPacketSend(Authenticatable target, AbstractPacket packet) {
                if (packet.getData() instanceof SourceHolder) {
                    ...
                    PlayerEntity player = (PlayerEntity) client.getUser().getOriginal();

                    // bump the source for the players chunk chunk
                    chunkTracker.get(locationToAudioChunkId(player.getBlockPos())).getContext().bump(source);
                }
            }
        };
    }
    ...
    public String locationToAudioChunkId(BlockPos location) {
        int chunkX = step(location.getX());
        int chunkZ = step(location.getZ());
        return chunkX + "@" + chunkZ;
    }
